...
test.describe('getByRole "button"', () => {
...
test.describe('Поиск элементов форм по ролям', () => {
  ...
  test('Заполнить и отправить форму', async ({ page }) => {
    await page.getByRole('textbox',{ name:"Имя пользователя" }).fill("New test User")
    await page.getByRole('textbox',{ name:"Пароль"}).fill("Password")
    await page.getByRole('combobox',{ name: "Страна"}).selectOption("us")
